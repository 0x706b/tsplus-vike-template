...
const collectFiles = (configPath: string) => {
  ...
  const tsconfig = ts.parseJsonConfigFileContent(config, ts.sys, baseDir!, undefined, configPath);
...
export const getCompiled = (path: string) => {
  ...
  const syntactic: Array<ts.DiagnosticWithLocation> = services.getSyntacticDiagnostics(path);
